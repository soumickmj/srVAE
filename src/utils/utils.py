...
def logging(epoch, train_losses, valid_losses, is_saved, writer=None):
    if writer is not None:
        for loss in train_losses:
            writer.add_scalar(f'Train Loss/{loss}', train_losses[loss], epoch)

        for loss in valid_losses:
            writer.add_scalar(f'Validation Loss/{loss}', valid_losses[loss], epoch)
...
def save_model(model, optimizer, loss, epoch, pth='./src/models/'):
    ...
    torch.save(m.state_dict(), f'{pth_inf}/model.pth')
...
def namespace2markdown(args, title='Hyperparameter', values='Values'):
    txt = f'<table> <thead> <tr> <td> <strong> {title} </strong> </td> <td> <strong> {values} </strong> </td> </tr> </thead>'

    txt += ' <tbody> '
    for name, var in vars(args).items():
        txt += f'<tr> <td> <code>{str(name)} </code> </td> <td> <code> {str(var)} </code> </td> <tr> '
